...
CREATE POLICY "Users can view their own auto-save rules" ON public.savings_auto_rules FOR SELECT USING (auth.uid() = user_id);
CREATE POLICY "Users can insert their own auto-save rules" ON public.savings_auto_rules FOR INSERT WITH CHECK (auth.uid() = user_id);
CREATE POLICY "Users can update their own auto-save rules" ON public.savings_auto_rules FOR UPDATE USING (auth.uid() = user_id);
CREATE POLICY "Users can delete their own auto-save rules" ON public.savings_auto_rules FOR DELETE USING (auth.uid() = user_id);
